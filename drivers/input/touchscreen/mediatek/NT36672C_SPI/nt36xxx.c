...
static struct work_struct nvt_charger_detect_work;
...
void nvt_touch_resume_workqueue_calback(struct work_struct *work)
{
	NVT_LOG("NVT TP RESUME START\n");
	nvt_ts_resume(&ts->client->dev);
	NVT_LOG("NVT TP RESUME end\n");
}

void nvt_touch_suspend_workqueue_calback(struct work_struct *work)
{
	NVT_LOG("NVT TP SUSPEND START\n");
	nvt_ts_suspend(&ts->client->dev);
	NVT_LOG("NVT TP SUSPEND end\n");
}

void nvt_charger_mode_notifier_callback(struct work_struct *work)
{
	u8 buf[8] = {0};
	if (nvt_charger_flag == TPD_DETECT_USB_IN) {
		buf[0] = EVENT_MAP_HOST_CMD;
		buf[1] = 0x53; /*USB IN*/
		CTP_SPI_WRITE(ts->client, buf, 2);
		NVT_LOG("nvt in charger mode\n");
	} else if (nvt_charger_flag == TPD_DETECT_USB_OUT){
		buf[0] = EVENT_MAP_HOST_CMD;
		buf[1] = 0x51; /*USB OUT*/
		CTP_SPI_WRITE(ts->client, buf, 2);
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	INIT_WORK(&nvt_touch_resume_work, nvt_touch_resume_workqueue_calback);

	nvt_touch_suspend_workqueue = create_singlethread_workqueue("nvt_touch_suspend");
	INIT_WORK(&nvt_touch_suspend_work, nvt_touch_suspend_workqueue_calback);

	nvt_charger_detect_workqueue = create_singlethread_workqueue("nvt_charger_detect");
	INIT_WORK(&nvt_charger_detect_work, nvt_charger_mode_notifier_callback);
	...
	ret = tpd_charger_detect_register_client(&ts->nvt_charger_detect_notif);
	if(ret) {
...
err_register_charger_nofif_failed:
	if(tpd_charger_detect_unregister_client(&ts->nvt_charger_detect_notif))
...
static int32_t __init nvt_driver_init(void)
{
	...
	if (IS_ERR_OR_NULL(mtkfb_lcm_name)){
		NVT_ERR("mtkfb_lcm_name ERROR!\n");
		ret = -ENOMEM;
		goto err_driver;
	} else {
		if (strcmp(mtkfb_lcm_name,"dsi_panel_m19a_36_02_0a_dsc_vdo_lcm_drv") == 0) {
			NVT_LOG("TP name: %s\n", mtkfb_lcm_name);
		} else {
